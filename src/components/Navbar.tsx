...
import { Moon, Shield, Sparkles, Wallet, RefreshCw, LogOut } from "lucide-react";
...
export const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab, onOpenCreateModal }) => {
  const { isConnected, selectedAccount, connectWallet, disconnectWallet } = useWallet();
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {isConnected && selectedAccount ? (
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-900/90 border border-indigo-900/50 text-xs text-slate-200">
                  ...
                  <div className="text-left max-w-[120px] sm:max-w-[150px] truncate">
                    <span className="font-semibold block truncate">
                      {selectedAccount.address.slice(0, 10)}...{selectedAccount.address.slice(-4)}
                    </span>
              ...
              <button
                onClick={() => connectWallet().catch((err) => alert(err.message || "Failed to connect wallet"))}
